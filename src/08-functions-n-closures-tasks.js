...
function getPolynom(...str) {
  return (x) => [...str].reduce((sum, n, i, a) => sum + n * (x ** (a.length - 1 - i)), 0);
}
...
function logger(func, logFunc) {
  return (...argums) => {
    const w = JSON.stringify(argums).slice(1, JSON.stringify(argums).length - 1);
    logFunc(`${func.name}(${w}) starts`);
    const result = func.apply(this, argums);
    logFunc(`${func.name}(${w}) ends`);
    return result;
  };
}
