...
function findFirstSingleChar(str) {
  ...
  chars.forEach((item) => {
    if (
      str.match(new RegExp(item, 'g')).length === 1 && !skip
    ) {
      skip = true;
      result = item;
    }
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${open}${(a < b) ? a : b}, ${(a < b) ? b : a}${close}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  const ccnCheck = +ccnStr[ccnStr.length - 1];
  const ccnArr = ccnStr.slice(0, ccnStr.length - 1).split('');
  const ccnSum = ccnArr
    .reverse()
    .map((item, index) => {
      let newItem;
    ...
    .reduce((prev, cur) => {
      const newCur = (cur.toString().length > 1)
        ? cur.toString().split('').reduce((curComp, prevComp) => +prevComp + +curComp, 0)
        : +cur;
      return prev + newCur;
    }, 0);
  return ((ccnSum + ccnCheck) % 10 === 0);
}
...
function isBracketsBalanced(str) {
  ...
  str.split('').forEach((el) => {
    if (keys.some((br) => br === el)) {
      stack.push(el);
    } else if (stack.some((br) => values.indexOf(el) === keys.indexOf(br))) {
      stack.pop();
    } else {
      stack.push(el);
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  return `${arrs[0].filter((ch, i) => arrs.every((strArr) => strArr[i] === ch)).join('/')}/`;
}
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    matrix[i] = new Array(m2[i].length);
